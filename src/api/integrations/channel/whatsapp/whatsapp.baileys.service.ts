    ...
    'messages.upsert': async (
      ...
      try {
        for (const received of messages) {
          ...
          // DO NOT REPLACE @lid WITH @s.whatsapp.net — keep original remoteJid for webhook routing
    ...
    const verifiedUsers = await Promise.all(
      jids.users.map(async (user) => {
        ...
        if (cached) {
          ...
          const effectiveJid = user.jid.includes('@lid') ? user.jid : cached.remoteJid;
          const contactName =
            contacts.find((c) => c.remoteJid === effectiveJid)?.pushName ||
          ...
          return new OnWhatsAppDto(
            ...
            user.number,
            contactName,
            user.jid.includes('@lid') ? 'lid' : cached.lid || (cached.remoteJid.includes('@lid') ? 'lid' : undefined),
          );
        }
